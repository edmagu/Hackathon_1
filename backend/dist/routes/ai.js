...
function getMockPlan(project) {
    ...
    const getMember = (i) => members.length > 0 ? members[i % members.length].name : 'Team Member';
    return {
        phases: [
            {
                id: (0, uuid_1.v4)(),
    ...
    try {
        ...
        if (!process.env.OPENAI_API_KEY) {
            ...
            const memberList = (project.teamMembers || [])
                .map((m) => `${m.name} (${m.role || 'member'}, skills: ${m.skills || 'general'})`)
